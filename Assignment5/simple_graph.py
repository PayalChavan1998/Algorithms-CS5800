...
# the neighbor list is a list of dictionaries
#   n["node"] is the Node of the neighbor
#   n["weight"] is the weight of the edge from self to n["node"]
...
class Node:
    ...
    def addUndirectedNeighbor( self, n, weight = 1 ):
        if not (n in self._neighbors):
            self._neighbors.append( {"node":n, "weight":weight} )
            n._neighbors.append( {"node":self, "weight":weight} )

    # adds a neighbor to this node's adjacency list
    def addDirectedNeighbor( self, n, weight = 1 ):
        if not (n in self._neighbors):
            self._neighbors.append( {"node":n, "weight":weight} )
